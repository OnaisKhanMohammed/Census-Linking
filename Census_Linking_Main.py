...
Clusters_with_ID_left=Blocking_Clustering(dataset1_2020,"2020","File_A_Parms.txt",Clusters_with_ID_left)
Clusters_with_ID_right=Blocking_Clustering(dataset2_2030,"2030","File_B_Parms.txt",Clusters_with_ID_right)

print(Clusters_with_ID_left)
print(Clusters_with_ID_right)
...
for i in tqdm(left_lines):
    line=i.strip("\n").split("|")
    left_data=line[0]
    Cluster_full_left=line[1]
    Cluster_left=line[1] .split("_")
    Cluster_id_left=Cluster_left[0]
    Cluster_dataset_left=Cluster_left[1]
    for j in right_lines:
        line=j.strip("\n").split("|")
        right_data=line[0]
        Cluster_full_right=line[1]
        Cluster_right=line[1] .split("_")
...
for p in tqdm(pairs_final):
     ...
     g.add_node(p[1],color='blue',title=p[1],label=p[1],shape="image", image ="https://github.com/Univ-Of-Arkansas-at-LITTLE-ROCK/CensusBureauNameAddress/blob/main/Household%20Graph/Image.png?raw=true",size=30)
     Nodes_Final.append([p[0],p[1]])
